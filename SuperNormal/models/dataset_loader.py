...
class Dataset:
    def __init__(self, conf):
        ...
        valid_world_mat_keys = [key for key in camera_dict.files if key.startswith('world_mat_')]
        valid_indices = set(int(key.split('_')[-1]) for key in valid_world_mat_keys)
        self.img_idx_list = [idx for idx in self.img_idx_list if idx in valid_indices]
    ...
    def gen_random_patches(self, num_patch, patch_H=3, patch_W=3):
        ...
        # print(f"img_idx_expand min/max: {img_idx_expand.min().item()}/{img_idx_expand.max().item()}")
        # print(f"patch_center_y_all min/max: {patch_center_y_all.min().item()}/{patch_center_y_all.max().item()}")
        # print(f"patch_center_x_all min/max: {patch_center_x_all.min().item()}/{patch_center_x_all.max().item()}")
        # print(f"self.V_inverse_all shape: {self.V_inverse_all.shape}")
...
#         if self.upsample_factor > 1:
#             # resize normal maps with upsample_factor
...
#         if self.upsample_factor > 1:
#             # resize mask with upsample_factor
#             self.masks_np = F.interpolate(
#                 torch.from_numpy(self.masks_np).permute(0, 3, 1, 2),
#                 scale_factor=self.upsample_factor,
#                 mode='nearest'
#             ).permute(0, 2, 3, 1).numpy()
#         self.masks_np = self.masks_np[..., 0]
...
#         normal = self.normals[img_idx_expand, patch_center_y_all, patch_center_x_all]  # (num_patch, patch_H, patch_W, 3)
#         V_inverse_patch_all = self.V_inverse_all[img_idx_expand, patch_center_y_all, patch_center_x_all]  # (num_patch, patch_H, patch_W, 3, 3)
#         mask = self.masks[img_idx_expand, patch_center_y_all, patch_center_x_all].unsqueeze(-1)     # (num_patch, patch_H, patch_W, 1)
...
#     def image_at(self, idx, resolution_level=4):
#         img = cv.imread(self.images_lis[idx])
#         resized_img = cv.resize(img, (self.W // resolution_level, self.H // resolution_level))
#         return resized_img.clip(0, 255)
