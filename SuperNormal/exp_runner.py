...
def normal_to_grayscale(normal_map):
    return torch.linalg.norm(normal_map, ord=2, dim=-1)
...
class Runner:
    ...
    def get_loss_weights(self):
        """Retorna los pesos actualizados basados en el progreso"""
        progress = self.iter_step / self.end_iter
        
        if progress < 0.6:
            return 1.0, 1.0, 1.0
        elif progress < 0.85 and progress >= 0.6:
            return 2.0, 0.05, 0.1
        else:
            return 2.0, 0.25, 0.25
    ...
    def train(self):
        ...
        for iter_i in pbar:
            ...
            # print("Shape rays_o_patch_center:", rays_o_patch_center.shape)  # debe ser (num_patch, 3)
            # print("Shape rays_d_patch_center:", rays_d_patch_center.shape)  # debe ser (num_patch, 3)

            # print("Ejemplo rays_o_patch_center[0]:", rays_o_patch_center[0].cpu().numpy())
            # print("Ejemplo rays_d_patch_center[0]:", rays_d_patch_center[0].cpu().numpy())
            ...
            if 'gradients' not in render_out or render_out['gradients'] is None:
                self.update_learning_rate()
                continue


            comp_normal = render_out['comp_normal']  # [N, 3, H, W]
            gradients = render_out['gradients']      # [N, ..., 3]
            comp_mask = render_out['weight_sum']     # [N, 1, H, W]
            ...
            if self.loss_type == 'l1':
                normal_loss = F.l1_loss(normal_error, torch.zeros_like(normal_error), reduction='mean') #/ mask_sum
            elif self.loss_type == 'l2':
                
                # SuperNormal Paper Normal Loss 
                # normal_loss = F.mse_loss(normal_error, torch.zeros_like(normal_error), reduction='sum') / mask_sum


                # New reduction method
                normal_loss = F.mse_loss(normal_error, torch.zeros_like(normal_error), reduction='mean') 

                # normal_loss = (normal_error.pow(2).sum() / (mask_sum))  # para L2
            ...
            mask_loss = F.binary_cross_entropy(comp_mask.clip(1e-7, 1.0 - 1e-7), mask)


            # normal_weight, mask_weight, eikonal_weight = self.get_loss_weights()
            # --- Total loss ---
            ...
            self.iter_step += 1
            self.update_learning_rate()
            torch.cuda.empty_cache()  # limpia VRAM después de la validación
            ...
            #     # Generar malla predicha siempre
            #     mesh_pred = self.generate_mesh(resolution=512)
            #     self.save_mesh(mesh_pred, f"mesh_pred_{self.iter_step}.ply")

            #     cd, fscore = -1, -1  # Valores por defecto si no hay GT

                # # Calcular métricas solo si hay GT
                # if self.dataset.mesh_gt is not None:
                #     if self.dataset.points_gt is None:
                #         self.dataset.mesh_gt.vertices = o3d.utility.Vector3dVector(
                #             (np.asarray(self.dataset.mesh_gt.vertices) - 
                #             self.dataset.scale_mats_np[0][:3, 3][None]) / self.dataset.scale_mats_np[0][0, 0])
                ...
                #     print(f'MAE (all views) {self.val_gradient_method}: {mae_allview:.5f}')
                #     print(f'MAE (test views) {self.val_gradient_method}: {mae_test_view:.5f}')
                #     with open(csv_file_path, 'a') as f:
                #         writer = csv.writer(f)
                #         writer.writerow([self.iter_step, mae_allview, mae_test_view, cd, fscore])
                # else:
                #     mae_allview = self.eval_mae(gradient_method="dfd")
                #     with open(csv_file_path, 'a') as f:
                #         writer = csv.writer(f)
                #         writer.writerow([self.iter_step, mae_allview, cd, fscore])
    ...
    def validate_normal_pixel_based(self, idx=-1, resolution_level=-1):
        ...
        # Guardar máscara como PNG (0 y 255)
        cv.imwrite(os.path.join(self.base_exp_dir,
                                'normals',  # o 'masks'
                                '{:0>8d}_{}_{}_mask.png'.format(self.iter_step, 0, idx)),
    ...
    def validate_normal_patch_based(self, idx=-1, resolution_level=-1, gradient_method="dfd"):
        ...
        # ---- Compute edges from the resized mask using Canny ----
        ...
        # Guardar máscara como PNG (0 y 255)
        cv.imwrite(os.path.join(normal_dir, '{:0>8d}_{}_{}_mask.png'.format(self.iter_step, 0, idx)),
                (mask_np.astype(np.uint8) * 255))
    ...
    #                    Exporting Mesh
    # =============================================================================
    def partial_convex_hull_bottom(self, mesh_o3d, ratio_upper=0.15, axis=1):
        vertices = np.asarray(mesh_o3d.vertices)
        faces = np.asarray(mesh_o3d.triangles)

        z_min, z_max = vertices[:, axis].min(), vertices[:, axis].max()
        print(f"Z min: {z_min}, Z max: {z_max}")
        # z_cut = z_min + ratio_upper * (z_max - z_min) - z_max/2
        z_cut = z_min + ratio_upper * (z_max - z_min) + z_max/6

        mask_upper = vertices[:, axis] > z_cut      # parte superior intacta
        mask_lower = vertices[:, axis] <= z_cut     # parte inferior para convex hull

        # Caras superiores: todos sus vértices están en la parte superior
        faces_upper = faces[np.all(mask_upper[faces], axis=1)]

        # Caras inferiores: todos sus vértices están en la parte inferior
        faces_lower = faces[np.all(mask_lower[faces], axis=1)]

        mesh_upper = o3d.geometry.TriangleMesh()
        mesh_upper.vertices = o3d.utility.Vector3dVector(vertices)
        mesh_upper.triangles = o3d.utility.Vector3iVector(faces_upper)

        mesh_lower_hull = None
        if len(faces_lower) > 0:
            # extraer los vértices usados en las caras inferiores
            lower_vertex_indices = np.unique(faces_lower.flatten())
            lower_vertices = vertices[lower_vertex_indices]

            # reindexar las caras
            index_map = {old_idx: new_idx for new_idx, old_idx in enumerate(lower_vertex_indices)}
            faces_lower_reindexed = np.array([[index_map[i] for i in face] for face in faces_lower])

            mesh_lower = o3d.geometry.TriangleMesh()
            mesh_lower.vertices = o3d.utility.Vector3dVector(lower_vertices)
            mesh_lower.triangles = o3d.utility.Vector3iVector(faces_lower_reindexed)

            mesh_lower_hull, _ = mesh_lower.compute_convex_hull()

        if mesh_lower_hull is not None:
            combined = mesh_upper + mesh_lower_hull
        else:
            combined = mesh_upper
        ...
        ms = pml.MeshSet()
        mesh_pml = pml.Mesh(vertices, faces)
        ms.add_mesh(mesh_pml, "shape")
        ...
        if not combined.is_empty():
            ...
            # Crear la malla en PyMeshLab
            mesh_pml = pml.Mesh(vertices, faces)

            # Crear un MeshSet y agregar la malla
            ms = pml.MeshSet()
            ms.add_mesh(mesh_pml, "shape")

            ms.generate_alpha_wrap()
            # Guardar la malla 
            # ms.apply_coord_taubin_smoothing(lambda_=0.5, mu=0.5, stepsmoothnum=2)
            for i in range(1):
            ...
            ms.meshing_isotropic_explicit_remeshing(iterations=2)

            ms.apply_coord_taubin_smoothing(lambda_=0.3, mu=0.3, stepsmoothnum=1)
            for i in range(2):
                ms.meshing_surface_subdivision_catmull_clark()

        else:
            print("La malla cargada está vacía o no es válida")

        ms.meshing_decimation_quadric_edge_collapse(targetfacenum=50000, preservenormal=True)
        return ms

    def remeshing(self, mesh_o3d, ratio_upper=0.25, axis=1):
        vertices = np.asarray(mesh_o3d.vertices)
        faces = np.asarray(mesh_o3d.triangles)
        mesh_pml = pml.Mesh(vertices, faces)
        ms = pml.MeshSet()
        ms.add_mesh(mesh_pml, "shape")
        # Verificar si la malla es válida
        ms.meshing_isotropic_explicit_remeshing(iterations=10)
        # ms.apply_coord_taubin_smoothing(lambda_=0.5, mu=0.5, stepsmoothnum=10)
        for i in range(2):
            ms.meshing_surface_subdivision_catmull_clark()
        ms.apply_coord_taubin_smoothing(lambda_=0.5, mu=0.5, stepsmoothnum=10)
    ...
    def validate_mesh(self, world_space=True, resolution=256, threshold=0.0):
        ...
        # mesh2isotropic = self.remeshing(mesh)
        # Calcular la envolvente convexa
        ms = self.partial_convex_hull_bottom(mesh)
        # Guardar la malla de la envolvente convexa
        mesh_path = os.path.join(save_dir, 'iter_{:0>8d}.ply'.format(self.iter_step))
        ms.save_current_mesh(f"{mesh_path.replace('.ply', '_convex.ply')}")
        # mesh2isotropic.save_current_mesh(f"{mesh_path.replace('.ply', '_remeshing.ply')}")
    ...
    @torch.no_grad()
    def generate_mesh(self, resolution=512):
        """
        Wrapper para compatibilidad. Solo llama a eval_geo y devuelve la malla.
        """
        # print("[INFO] generate_mesh() llamado → ejecutando eval_geo()")
        print(f"Ready")
        # return self.eval_geo(resolution=resolution)
        return None
    @torch.no_grad()
    def save_mesh(self, mesh, filename):
        ...
        if isinstance(mesh, trimesh.Trimesh):
            ...
            raise TypeError("El tipo de mesh no es trimesh.Trimesh ni open3d.geometry.TriangleMesh")

        print(f"Mesh guardada en {save_path}")
    # @torch.no_grad()
    ...
    def eval_geo(self, resolution=1024):
        ...
        has_gt = hasattr(self.dataset, 'points_gt') and self.dataset.points_gt is not None

        bound_min = torch.tensor(self.dataset.object_bbox_min, dtype=torch.float32)
        bound_max = torch.tensor(self.dataset.object_bbox_max, dtype=torch.float32)

        vertices, triangles = self.renderer.extract_geometry(bound_min, bound_max, resolution=resolution, threshold=0)
        ...
        points_eval = self.find_visible_points(mesh_world)
        points_eval = points_eval * self.dataset.scale_mats_np[0][0, 0] + self.dataset.scale_mats_np[0][:3, 3][None]

        pcd_eval = o3d.geometry.PointCloud()
        pcd_eval.points = o3d.utility.Vector3dVector(points_eval)
        eval_path = os.path.join(save_dir, f"{self.iter_step}_points_eval.ply")
        o3d.io.write_point_cloud(eval_path, pcd_eval)

        if has_gt:
            pcd_gt = o3d.geometry.PointCloud()
            pcd_gt.points = o3d.utility.Vector3dVector(self.dataset.points_gt)
            gt_path = os.path.join(save_dir, "pcd_gt.ply")
            ...
            # Si no hay GT, devuelve métricas neutras para no romper nada en el entrenamiento
            return 0.0, 0.0
