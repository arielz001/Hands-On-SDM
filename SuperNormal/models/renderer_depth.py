...
def extract_fields(bound_min, bound_max, resolution, query_func):
    ...
    u_smoothed = gaussian_filter(u, sigma=2.0)
    return u_smoothed
...
class NeuSRenderer:
    ...
    def render(self, rays_o_patch_all,  # (num_patch, patch_H, patch_W, 3)
        ...
        if gradient_method == "dfd":
            ...
            # #######################################################
            # valid_pixel_mask = (weights_cuda.sum(dim=0) > 0).squeeze(-1)  # (patch_H, patch_W)
            # gradients = gradients * valid_pixel_mask[None, ..., None].float()
        ...
        t_centers_patch_all = 0.5 * (t_starts_patch_all + t_ends_patch_all)

        comp_depth_cuda = accumulate_along_rays_patch_based(
            weights_cuda, patch_indices,
            values=t_centers_patch_all.reshape(num_samples, patch_H * patch_W, 1),
            n_patches=num_patch
        )
        pred_depth = comp_depth_cuda.reshape(num_patch, patch_H, patch_W, 1)
