...
class Dataset:
    def __init__(self, conf):
        ...
        self.depth_lis = sorted(glob(os.path.join(self.data_dir, self.depth_dir, '*.png')))
        ...
        # self.depth_lis = sorted(glob(os.path.join(self.depth_dir, '/*.png')))

        with ThreadPoolExecutor(max_workers=min(64, os.cpu_count()*5)) as executor:
            def read_mask(im_name):
                return cv.imread(im_name)
            def read_depth(im_name):
                return cv.imread(im_name, cv.IMREAD_ANYDEPTH)
            
            self.masks_np = np.stack(list(executor.map(read_mask, self.masks_lis))) / 255.0
            self.depth_np = np.stack(list(executor.map(read_depth, self.depth_lis))) #/ 255.0
        ...
        if self.upsample_factor > 1:
            ...
            self.depth_np = F.interpolate(
                torch.from_numpy(self.depth_np[:, None, :, :]),  # [N, 1, H, W]
                scale_factor=self.upsample_factor,
                mode='bilinear',
                align_corners=False
            ).squeeze(1).numpy()  # [N, H', W']
        ...
        # i ADD DEPTHS HERE 
        self.depth = torch.from_numpy(self.depth_np.astype(np.float32)).to(self.device) # [n_images, H, W, 3]
    ...
    def gen_random_patches(self, num_patch, patch_H=3, patch_W=3):
        ...
        depth = self.depth[img_idx_expand, patch_center_y_all, patch_center_x_all]  # (num_patch, patch_H, patch_W, 3)
